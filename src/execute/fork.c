...
// 		run(salt, info, envp);
// 	if (salt->indentifier == CMD_BLOCK && salt->next->indentifier == INPUT_F && salt->next->next == NULL)
// 		input_first(fd, salt, info, envp);
// 	if (salt->indentifier == CMD_BLOCK && salt->next->indentifier == OUTPUT_F && salt->next->next == NULL)
