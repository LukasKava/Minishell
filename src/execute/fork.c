...
/*   Updated: 2022/11/26 19:46:42 by pbiederm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	third_child(t_chunk **salt, t_info *info, char	**envp)
{
	...
	num_pipes = 1;
	local_chunk = *salt;
	fprintf(stderr,"Before parsing local chunk\n");
	while(local_chunk != NULL)
	{
	...
	fprintf(stderr,"After parsing local chunk\n");
	vars.num_pipes = num_pipes - 1;
	// s = 0;
	local_chunk = *salt;
	// local_chunk = local_chunk->next;
	// while(local_chunk != NULL)
		...
		if (local_chunk->indentifier == CMD_BLOCK)
		{
			...
			if (pids[i] == 0)
			{
				...
				else
				{
					...
					fprintf(stderr,"set input from fd[%d]\n",i);
					close(fd[i][OUTPUT]);
				...
				{
					fprintf(stderr,"set output for fd[%d]\n",(i + 1));
					close(fd[i + 1][INPUT]);
